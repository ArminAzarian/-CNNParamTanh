...
# 1.3 CNN-based Parametric Dynamic Tanh
class CNNParamTanh(nn.Module):
    def __init__(self, in_channels, num_filters, video_height, video_width):
        super().__init__()
        ...
        #Need to calculate the correct size of the Linear Layer. I assume the image will be reduced twice by factor 2 by the MaxPool2d Layers.
        self.linear_size = (video_height // 4) * (video_width // 4) * num_filters * 2
        self.parameter_predictor = nn.Linear(self.linear_size, video_height*video_width)
    ...
    def forward(self, x, video):
      #Reshape video. The video is (B, T, C, H, W), and it needs to be (B, T*C, H, W) to pass it into a 2D conv
      video_reshape = reshape_video_to_channels(video)
      # Pass the video through the CNN
      cnn_output = self.cnn(video_reshape)
      cnn_output = torch.flatten(cnn_output, start_dim=1)
      # Predict the parameters
      alpha = self.parameter_predictor(cnn_output)
      B = x.shape[0]
...
class VAE(nn.Module):
    ...
    def forward(self, x, tanh_layer = None):
        ...
        if tanh_layer is not None:
            #We have a single image, we will create a video to feed it to the CNN
