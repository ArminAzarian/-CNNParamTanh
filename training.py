...
from model_definitions import VAE, Discriminator, combined_loss, create_synthetic_video, CNNParamTanh
...
# CNN Hyperparameters
CNN_IN_CHANNELS = NUM_FRAMES * CHANNELS
CNN_NUM_FILTERS = 2
...
# --- Training Loop ---
def train_cnn_tanh(device="cpu"):

    # --- Model Initialization ---
    tanh_layer = CNNParamTanh(CNN_IN_CHANNELS, CNN_NUM_FILTERS, VIDEO_HEIGHT, VIDEO_WIDTH).to(device)
    vae = VAE().to(device)
    ...
    optimizer_cnn = optim.Adam(tanh_layer.parameters(), lr=LEARNING_RATE)
    ...
    print(f"Training complete for CNNParamTanh model")

# --- Main execution ---
if __name__ == "__main__":
    device = "cuda" if torch.cuda.is_available() else "cpu"
    train_cnn_tanh(device)
